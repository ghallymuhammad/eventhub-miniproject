...
function CheckoutContent() {
  ...
  const [selectedTickets, setSelectedTickets] = useState([
    { type: 'General', quantity: 2, price: 250000 },
    { type: 'VIP', quantity: 1, price: 500000 }
  ]);
  ...
  // Mock event data - would normally fetch based on eventId
  const getEventData = (id: string | null) => {
    const events: { [key: string]: any } = {
      ...
      '3': {
        ...
        date: '2025-11-05',
        time: '10:00',
        location: 'Yogya Art Gallery',
        organizer: 'Yogya Art Gallery'
      }
    };
    
    return events[id || '1'] || events['1'];
  };
  ...
  const proceedToPayment = () => {
    // Here you would typically redirect to payment gateway or show payment options
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* Selected Tickets */}
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-xl font-semibold text-gray-900 mb-4">Selected Tickets</h2>
              <div className="space-y-4">
                {selectedTickets.map((ticket, index) => (
                  <div key={index} className="flex justify-between items-center py-3 border-b border-gray-200 last:border-0">
                    <div>
                      <h3 className="font-medium text-gray-900">{ticket.type} Ticket</h3>
                      <p className="text-sm text-gray-600">Quantity: {ticket.quantity}</p>
                    </div>
                    <div className="text-right">
                      <div className="font-medium text-gray-900">{formatCurrency(ticket.price * ticket.quantity)}</div>
                      <div className="text-sm text-gray-500">{formatCurrency(ticket.price)} each</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            <div className="bg-white rounded-lg shadow-md p-6 sticky top-8">
              ...
              >
                {timeRemaining <= 0 ? 'Time Expired' : 'Proceed to Payment'}
              </button>
