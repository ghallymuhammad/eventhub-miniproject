...
function CreateEventPage() {
  ...
  const [newReferralVoucher, setNewReferralVoucher] = useState({
    code: '',
    discountValue: 10,
    validDays: 30
  });
  ...
  const steps = [
    ...
    { id: 2, name: 'Tickets & Pricing', description: 'Set up ticket types and pricing' },
    { id: 3, name: 'Promotions', description: 'Create vouchers and promotions' },
    { id: 4, name: 'Review & Publish', description: 'Review and publish your event' }
  ];
  ...
  const validateStep = (step: number): boolean => {
    ...
    if (step === 2) {
      if (!eventData.isFree) {
        const hasValidTickets = eventData.ticketTypes.some(ticket => 
          ticket.name.trim() && ticket.price >= 0 && ticket.seats > 0
        );
        if (!hasValidTickets) {
          newErrors.tickets = 'At least one valid ticket type is required for paid events';
        }
      }
    }
